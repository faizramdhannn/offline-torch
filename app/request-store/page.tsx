...
const VAPID_PUBLIC_KEY = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || "BPRJgdT0HHzl7GeptF_hhQ4JncvHV2AzNfdihGrDrd3FEvjIZK_T-t7_1Ggib3UTMA9OYuVyrdnx6X7xWmveLZY";
...
export default function RequestStorePage() {
  ...
  const handleStatusChange = async (item: RequestItem, newStatus: string) => {
    ...
    } catch { showMessage("Gagal update status", "error"); }
  ...
  const handleDelete = async (item: RequestItem) => {
    ...
    } catch { showMessage("Gagal menghapus request", "error"); }
  };

  const openEdit = (item: RequestItem) => {
    setSelectedItem(item);
    setEditForm({ date: item.date, requester: item.requester, assigned_to: item.assigned_to, reason_request: item.reason_request, notes: item.notes || "", status: item.status });
                      ...
                      {[...Array(totalPages)].map((_, i) => {
                        ...
                        if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
                          return <button key={page} onClick={() => setCurrentPage(page)}
                            className={`px-3 py-1 text-xs border rounded ${currentPage === page ? "bg-primary text-white" : "hover:bg-gray-50"}`}>{page}</button>;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => { setShowAddModal(false); setForm({ date: new Date().toISOString().split("T")[0], requester: "", assigned_to: "", reason_request: "", notes: "" }); }}
