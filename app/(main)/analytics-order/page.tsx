        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="px-5 py-4 grid grid-cols-2 gap-3 border-b">
            ...
            <div className="bg-gray-50 rounded-xl px-3 py-2.5">
              ...
              {online ? (
                <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 text-[11px] font-bold border border-blue-200">🛒 Online</span>
              ) : trafficCode ? (
...
function MasterTrafficModal({
  ...
  const openAdd = () => {
    setFormMode("add"); setEditTarget(null); setFormCode(""); setFormNotes(""); setFormError("");
    setTimeout(() => codeRef.current?.focus(), 60);
  };

  const openEdit = (entry: { code_traffic: string; notes: string }) => {
    setFormMode("edit"); setEditTarget(entry); setFormCode(entry.code_traffic); setFormNotes(entry.notes); setFormError("");
    setTimeout(() => codeRef.current?.focus(), 60);
  };
          ...
          <div className="px-5 py-3 bg-blue-50 border-b flex-shrink-0">
            ...
            <div className="flex gap-2 items-end">
              <div>
                ...
                <input ref={codeRef} type="text" value={formCode} onChange={(e) => setFormCode(e.target.value.toUpperCase())}
                  maxLength={10} placeholder="WG"
...
useEffect(() => {
  ...
  const filteredRows = useCallback(() => {
    return rows.filter((r) => {
      ...
      if (activeTab === "online") {
        return isOnlineOrder(r.Notes);
      }

      // All other tabs: exclude online orders
      if (isOnlineOrder(r.Notes)) return false;
  ...
  const onlineCountInRange = (() => {
    return new Set(
      rows.filter((r) => {
        ...
        return isOnlineOrder(r.Notes) && r.Name;
      }).map((r) => r.Name)
    ).size;
  })();
  ...
  const dailyRevenueByStore = (() => {
    ...
    const chartData = Object.entries(map)
      ...
      .map(([date, storeRevMap]) => ({
        date: formatShortDate(date), fullDate: date,
  ...
  const trafficData = (() => {
    ...
    let nullCount = 0; let nullSubtotal = 0;
    const orderSeen = new Set<string>();
    fr.forEach((r) => {
      const key = r.Name || "";
      if (orderSeen.has(key)) return;
      ...
      if (code) {
        ...
        map[label].count++; map[label].subtotal += sub;
      } else { nullCount++; nullSubtotal += sub; }
    });
  ...
  const discountData = (() => {
    ...
    const orderSeen = new Set<string>();
    let noDiscountCount = 0; let noDiscountSubtotal = 0;
    fr.forEach((r) => {
      const key = r.Name || "";
      if (orderSeen.has(key)) return;
      orderSeen.add(key);
      const code = r["Discount Code"]?.trim();
      if (!code) { noDiscountCount++; noDiscountSubtotal += parseSubtotal(r.Subtotal); return; }
      if (!map[code]) map[code] = { count: 0, total: 0, subtotal: 0 };
      map[code].count++; map[code].total += parseSubtotal(r["Discount Amount"]); map[code].subtotal += parseSubtotal(r.Subtotal);
    });
    const result = Object.entries(map)
      .map(([name, d]) => ({ name, count: d.count, total: d.total, subtotal: d.subtotal }))
      .sort((a, b) => b.count - a.count).slice(0, 20);
    if (noDiscountCount > 0) result.push({ name: "Tanpa Discount", count: noDiscountCount, total: 0, subtotal: noDiscountSubtotal });
    return result;
  })();
  ...
  const productData = (() => {
    ...
    fr.forEach((r) => {
      ...
      map[name].qty += qty; map[name].revenue += price * qty;
    });
  ...
  const employeeData = (() => {
    ...
    fr.forEach((r) => {
      ...
      if (!orderSeen.has(key)) { orderSeen.add(key); map[emp].subtotal += parseSubtotal(r.Subtotal); }
    });
  ...
  const dailyEmployeeData = (() => {
    ...
    fr.forEach((r) => {
      ...
      if (!seen.has(key)) { seen.add(key); map[date][emp] = (map[date][emp] || 0) + parseSubtotal(r.Subtotal); }
    });
    return {
      chartData: Object.keys(map).sort().map((date) => ({
        date: formatShortDate(date), fullDate: date,
  ...
  const onlineOrderData = (() => {
    if (activeTab !== "online") return {
      ...
      totalRevenue: 0, totalOrders: 0, dailyData: [] as { date: string; fullDate: string; revenue: number; orders: number }[],
    ...
    fr.forEach((r) => {
      ...
      if (!key) return;
      if (!orderMap[key]) {
        const paidAt = r["Paid at"] || r["Created at"] || "";
        orderMap[key] = {
          name: key,
          date: paidAt.split(" ")[0],
      ...
      if (r["Lineitem name"]?.trim()) orderMap[key].itemCount++;
    });
    ...
    orders.forEach((o) => {
      if (!o.date) return;
      if (!dailyMap[o.date]) dailyMap[o.date] = { revenue: 0, orders: 0 };
    ...
    orders.forEach((o) => {
      ...
      storeMap[o.store].value += o.subtotal;
      storeMap[o.store].count++;
    });
    const revenueByStore = Object.entries(storeMap)
      ...
      <div className="p-4">
        <div className="flex-1 overflow-auto">
          <div className="p-6">
            ...
            {/* Summary Cards — hanya tampil di tab offline */}
            {activeTab !== "online" && (
              <>
                <div className="grid grid-cols-3 gap-4 mb-4">
                  {[
                    { label: "Total Revenue (Offline)", value: formatRupiah(totalRevenue), color: "text-green-600" },
                ...
                {/* Banner info online orders */}
                {onlineCountInRange > 0 && (
                  <div
                    className="mb-4 flex items-center gap-2 text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 cursor-pointer hover:bg-blue-100 transition-colors"
                  ...
                  >
                    ...
                    <span>
                      <strong>{onlineCountInRange} order online</strong> dipisah dan tidak dihitung di sini. Klik untuk lihat tab Online →
                    </span>
                  </div>
                )}
              </>
            ...
            {/* Summary Cards untuk tab Online */}
            {activeTab === "online" && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                  <p className="text-xs text-blue-500 mb-1">Total Order Online</p>
                  <p className="text-xl font-bold text-blue-700">{onlineOrderData.totalOrders.toLocaleString()}</p>
                </div>
                <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                  <p className="text-xs text-green-500 mb-1">Total Revenue Online</p>
                  <p className="text-xl font-bold text-green-700">{formatRupiah(onlineOrderData.totalRevenue)}</p>
                </div>
              </div>
            )}
            ...
            <GlassCard className="mb-4">
              <div className="grid grid-cols-2 gap-3 items-end sm:grid-cols-3 lg:grid-cols-5">
                ...
                {activeTab !== "online" ? (
                  <div>
                    ...
                    <div className="relative" ref={trafficDropdownRef}>
                      <button
                        ...
                        className={`w-full px-2 py-1.5 border rounded text-xs text-left flex items-center justify-between gap-1 focus:outline-none focus:ring-1 focus:ring-primary transition-colors ${
                          isTrafficActive ? "border-primary bg-primary/5 text-primary font-medium" : "border-gray-300 bg-white text-gray-700"
                        } disabled:opacity-50`}
                      >
                        <span className="truncate flex items-center gap-1.5">
                          {trafficMapLoading ? "Memuat..." : isTrafficActive ? (
                            <>
                              <span className="inline-flex items-center justify-center w-4 h-4 bg-primary text-white rounded-full text-[9px] font-bold flex-shrink-0">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          {isTrafficActive && (
                            <span onClick={(e) => { e.stopPropagation(); clearTrafficFilter(); }}
                              className="text-primary/60 hover:text-red-500 cursor-pointer text-sm leading-none" title="Hapus semua filter traffic">×</span>
                          )}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-400 mb-1">Traffic Source</label>
                    <div className="w-full px-2 py-1.5 border border-gray-200 rounded text-xs text-gray-400 bg-gray-50 italic">
                      N/A (tab online)
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mt-1 w-full">
                    ...
                    {trafficFilter.map((code) => (
                      <span key={code}
                        ...
                        <button onClick={() => toggleTrafficCode(code)} className="ml-0.5 hover:text-red-500 leading-none">×</button>
                      </span>
                    ))}
                    {trafficFilter.length > 1 && (
                      <button onClick={clearTrafficFilter} className="text-[10px] text-red-500 hover:text-red-700 underline">
                        Hapus semua
                      </button>
                    )}
                  </div>
            ...
            <GlassCard padding="none" className="overflow-hidden">
              <div className="flex items-center justify-between border-b pr-4">
                <div className="flex overflow-x-auto">
                  {CHART_TABS.map((tab) => (
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id); resetPages(); }}
                ...
                <div className="flex items-center gap-3">
                  {!loading && fr.length > 0 && (
                    <>
                      {activeTab !== "online" && <ViewToggle view={chartView} onChange={setChartView} />}
                            ...
                            <div>
                              ...
                              <ResponsiveContainer width="100%" height={280}>
                                <BarChart data={revenueByStore} margin={{ top: 16, right: 8, left: 0, bottom: 40 }}>
                                  ...
                                  <YAxis tick={{ fontSize: 9, fill: "#334155" }} tickFormatter={(v) => v >= 1e6 ? `${(v/1e6).toFixed(0)}jt` : v >= 1e3 ? `${(v/1e3).toFixed(0)}k` : v} width={50} />
                            ...
                            <div>
                              ...
                              <ResponsiveContainer width="100%" height={240}>
                                <BarChart data={orderCountByStore.sort((a,b) => b.count - a.count)} margin={{ top: 8, right: 8, left: 0, bottom: 40 }}>
                                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#94a3b8" />
                                  <XAxis dataKey="name" tick={{ fontSize: 10, fill: "#6b7280" }} angle={-30} textAnchor="end" interval={0} />
                                  <YAxis tick={{ fontSize: 9, fill: "#334155" }} width={36} />
                            ...
                            <div>
                              ...
                              <ResponsiveContainer width="100%" height={220}>
                                <BarChart data={onlineOrderData.dailyData} margin={{ top: 8, right: 8, left: 0, bottom: 40 }}>
                                  ...
                                  <YAxis tick={{ fontSize: 9, fill: "#334155" }} width={36} />
                                  <Tooltip content={<DarkTooltip />} />
                                  <Bar dataKey="orders" name="Orders" fill="#06b6d4" radius={[4, 4, 0, 0]} maxBarSize={40} />
                                </BarChart>
                              </ResponsiveContainer>
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                              <BarChart data={onlineOrderData.revenueByStore} margin={{ top: 8, right: 8, left: 0, bottom: 40 }}>
                                ...
                                <Tooltip content={<DarkTooltip formatter={formatRupiah} />} />
                                <Bar dataKey="value" name="Revenue" radius={[4, 4, 0, 0]} maxBarSize={48}>
                                  {onlineOrderData.revenueByStore.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                                </Bar>
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
...
function TrafficMultiSelect({
  ...
  trafficFilter: string[]; trafficMap: Record<string, string>;
  onToggle: (code: string) => void; onClearAll: () => void;
}) {
  const [search, setSearch] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);
  ...
  const filtered = search.trim()
    ? allOptions.filter(o => o.displayLabel.toLowerCase().includes(search.toLowerCase()) || o.code.toLowerCase().includes(search.toLowerCase()))
    : allOptions;
      ...
      <div className="max-h-56 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          filtered.map((o) => {
            ...
            return (
              <button key={o.code} onClick={() => onToggle(o.code)}
                className={`w-full text-left px-3 py-2 text-xs hover:bg-primary/5 transition-colors flex items-center gap-2.5 ${isChecked ? "bg-primary/5" : ""}`}>
                <span className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center transition-colors ${isChecked ? "bg-primary border-primary" : "border-gray-300 bg-white"}`}>
                  {isChecked && (
                    <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className={`${isChecked ? "text-primary font-medium" : "text-gray-700"}`}>{o.displayLabel}</span>
              </button>
            );
          })
        )}
