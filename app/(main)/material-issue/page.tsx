      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3.5">
          {[
            {
              label: "Status",
              content: <ProcessedBadge value={item.has_processed} />,
            },
            {
              label: "SKU",
              content: (
            ...
            },
            {
              label: "Created By",
              content: (
                <span className="text-xs text-gray-800">{item.created_by}</span>
              ),
            },
            {
              label: "Update By",
            ...
            <div key={label} className="flex items-start justify-between gap-4">
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide shrink-0 w-24">
                {label}
              </span>
...
// ─── SKU Scanner Section (embedded in Add Modal) ──────────────────────────────
function SkuScannerSection({
  ...
  // Auto-focus hidden input for scanner mode
  useEffect(() => {
    if (scanMode === "scanner") {
  ...
  // Re-focus hidden input whenever user clicks anywhere in scanner mode
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        await qr.start(
          ...
          (decodedText: string) => {
            if (!cancelled) processSku(decodedText);
          },
          () => {}
        );
  ...
  const handleHiddenInput = (val: string) => {
  ...
  const removeItem = (sku: string) =>
    onItemsChange(scannedItems.filter((i) => i.sku !== sku));
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Scanner mode: invisible input + visual indicator */}
      {scanMode === "scanner" && (
        <>
          <input
            ref={hiddenInputRef}
            ...
            autoComplete="off"
          />
          <div className="flex items-center justify-center gap-2 py-3 border-2 border-dashed border-primary/40 rounded-lg bg-primary/5">
...
export default function MaterialIssuePage() {
  ...
  useEffect(() => {
    ...
    if (!u.material_issue && !u.material_issue_all) {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(
        `/api/material-issue?userName=${encodeURIComponent(
          user?.user_name || ""
        )}&isAll=${!!user?.material_issue_all}`
      );
      if (res.ok) {
  ...
  const handleToggleProcessed = async (item: MIItem) => {
    ...
    setData((prev) =>
      prev.map((d) =>
        d.id === item.id ? { ...d, has_processed: newVal } : d
      )
    );
    try {
      ...
      if (!res.ok) {
        setData((prev) =>
          prev.map((d) =>
            d.id === item.id ? { ...d, has_processed: item.has_processed } : d
          )
        );
  ...
  const handleAdd = async () => {
    if (
      ...
      showMessage(
        "Semua field wajib diisi dan minimal 1 item di-scan",
        "error"
      );
      return;
    }
  ...
  const handleDelete = async (item: MIItem) => {
    ...
    try {
      const res = await fetch(`/api/material-issue?id=${item.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
                      ...
                      return (
                        ...
                        >
                          <td className="px-1.5 py-1 border-r border-gray-200">
                            <div className="truncate font-mono text-[9px] text-gray-600 font-bold">
                              {item.id}
                            </div>
                          ...
                          <td className="px-1.5 py-1 border-r border-gray-200 truncate text-gray-600">
                            {hasSearch
                              ? highlightText(item.request_by, search)
                              : item.request_by}
                          </td>
                          <td
                            className="px-1.5 py-1 border-r border-gray-200"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <div className="flex items-center gap-0.5">
                              <span className="font-mono text-[9px] truncate flex-1">
                                {hasSearch
                                  ? highlightText(item.request_number, search)
                                  : item.request_number}
                              </span>
                              {item.request_number && (
                                <CopyButton
                                  text={item.request_number}
                                  id={`rn-${item.id}-${item.item_sku}`}
                                  copiedId={copiedId}
                                  onCopy={handleCopy}
                                />
                              )}
                          ...
                          >
                            <div className="flex items-center gap-0.5">
                              <span className="font-mono text-[9px] truncate flex-1">
                                {hasSearch
                                  ? highlightText(item.issue_number, search)
                                  : item.issue_number}
                              </span>
                              {item.issue_number && (
                                <CopyButton
                                  text={item.issue_number}
                                  id={`in-${item.id}-${item.item_sku}`}
                                  copiedId={copiedId}
                                  onCopy={handleCopy}
                                />
                              )}
                          ...
                          <td
                            className="px-1.5 py-1 border-r border-gray-200 truncate text-gray-600"
                            title={item.reason}
                          >
                            {item.reason || "-"}
                          </td>
                          ...
                          >
                            ...
                            >
                              {isProcessed ? (
                                ...
                                <svg
                                  className="w-3.5 h-3.5 text-gray-300 hover:text-gray-400"
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
                                >
              ...
              {totalPages > 1 && (
                <div className="flex justify-between items-center px-4 py-2.5 border-t">
                  <div className="text-xs text-gray-500">
                    {indexOfFirst + 1}–
                    {Math.min(indexOfFirst + itemsPerPage, filteredData.length)}{" "}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, request_by: e.target.value })
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, type_reason: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  >
                    <option value="">Pilih</option>
                    {dropdownData.type_reason.map((t) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, request_number: e.target.value })
                    }
              ...
              <div>
                ...
                <textarea
                  value={form.reason}
                  onChange={(e) =>
                    setForm({ ...form, reason: e.target.value.slice(0, 50) })
                  }
                  rows={2}
                  maxLength={50}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, item_sku: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, item_qty: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, request_by: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  >
                    <option value="">Pilih</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, type_reason: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  >
                    <option value="">Pilih</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      reason: e.target.value.slice(0, 50),
                    })
                  }
                  rows={2}
                  maxLength={50}
                ...
                <div className="flex gap-3">
                  {["FALSE", "TRUE"].map((v) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setEditForm({ ...editForm, has_processed: v })
                      }
