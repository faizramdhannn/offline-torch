              ...
              return (
                <tr key={store} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  ...
                  {matrix.sources.map((src) => {
                    ...
                    return (
                      <td key={src} className="border border-gray-200 px-2 py-1 text-center">
                        {val > 0 ? <span className="font-semibold text-blue-700">{val}</span> : <span className="text-gray-300">-</span>}
                      </td>
                    );
                  })}
                  <td className="border border-gray-200 bg-green-50/50 px-2 py-1 text-center font-semibold text-green-700">{storeBeli || "-"}</td>
                  <td className="border border-gray-200 bg-red-50/50 px-2 py-1 text-center font-semibold text-red-600">{storeTidak || "-"}</td>
                  <td className="border border-gray-200 bg-primary/5 px-2 py-1 text-center font-bold text-primary">{rowTotal}</td>
                </tr>
              );
            })}
            <tr className="bg-primary/10 font-bold">
              <td className="border border-gray-200 px-2 py-1 text-gray-700">TOTAL</td>
              {matrix.sources.map((src) => {
                const colTotal = matrix.stores.reduce((s, store) => s + (matrix.map[store]?.[src] || 0), 0);
                return (
                  <td key={src} className="border border-gray-200 px-2 py-1 text-center text-primary">
                    {colTotal}
                  </td>
                );
              })}
              <td className="border border-gray-200 px-2 py-1 text-center text-green-700">{totalBeli}</td>
              <td className="border border-gray-200 px-2 py-1 text-center text-red-600">{totalTidakBeli}</td>
              <td className="border border-gray-200 px-2 py-1 text-center text-primary">{totalEntries}</td>
            </tr>
          </tbody>
        </table>
      </div>
